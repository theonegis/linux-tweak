...
# Reapply the frameless ChatGPT titlebar patch after rebuilding and installing
# codex-desktop-linux from Git. The patcher is deliberately conservative:
# unknown upstream layouts abort before any installed file is changed.

readonly source_root="${CHATGPT_DESKTOP_SOURCE_ROOT:-$HOME/Developer/codex-desktop-linux}"
readonly package_name="${CHATGPT_DESKTOP_PACKAGE:-codex-desktop}"
readonly app_root="${CHATGPT_DESKTOP_ROOT:-/opt/codex-desktop}"
readonly asar_file="$app_root/resources/app.asar"
readonly assets_root="$app_root/content/webview/assets"
readonly state_root="${XDG_STATE_HOME:-$HOME/.local/state}/chatgpt-frameless-titlebar"
...
case "${1:-}" in
    ...
    -h|--help)
        printf '%s\n' \
            ...
            "  CHATGPT_DESKTOP_SOURCE_ROOT  Git checkout (default: $source_root)" \
            "  CHATGPT_DESKTOP_ROOT         installed app (default: $app_root)" \
            "  CHATGPT_DESKTOP_PACKAGE      pacman package (default: $package_name)"
        exit 0
        ;;
    *)
...
for command_name in asar git node rg pacman sha256sum sudo; do
...
if [[ ! -d "$source_root/.git" ]]; then
    printf 'The codex-desktop-linux Git checkout was not found at %s\n' "$source_root" >&2
    exit 1
fi

if [[ ! -r "$asar_file" || ! -d "$assets_root" ]]; then
    printf 'The installed ChatGPT application was not found below %s\n' "$app_root" >&2
    exit 1
fi

package_version="$(pacman -Q "$package_name" 2>/dev/null | awk '{print $2}')"
if [[ -z "$package_version" ]]; then
    printf 'The pacman package %s is not installed.\n' "$package_name" >&2
    exit 1
fi

owned_package="$(pacman -Qqo "$asar_file" 2>/dev/null || true)"
if [[ "$owned_package" != "$package_name" ]]; then
    printf '%s is owned by %s, not the expected package %s.\n' \
        "$asar_file" "${owned_package:-no pacman package}" "$package_name" >&2
    exit 1
fi

checkout_commit="$(git -C "$source_root" rev-parse --short=12 HEAD 2>/dev/null)"
build_info_file="$app_root/.codex-linux/build-info.json"
built_commit=""
if [[ -r "$build_info_file" ]]; then
    built_commit="$(node -e '
const fs = require("node:fs");
const value = JSON.parse(fs.readFileSync(process.argv[1], "utf8"));
process.stdout.write(value?.source?.shortCommit ?? "");
' "$build_info_file")"
fi
if [[ -n "$built_commit" && "$built_commit" != "$checkout_commit" ]]; then
    printf 'Warning: installed ChatGPT was built from Git commit %s; checkout is now %s.\n' \
        "$built_commit" "$checkout_commit" >&2
fi
...
restore_backup() {
    if chatgpt_is_running; then
        printf 'ChatGPT is still running. Fully quit it, including its tray process.\n' >&2
        exit 1
    fi
    ...
    printf 'Restored the original files for %s %s.\n' "$package_name" "$package_version"
}
...
if [[ "$mode" == "patch" ]] && chatgpt_is_running; then
    printf 'ChatGPT is still running. Fully quit it, including its tray process, then rerun this script.\n' >&2
    exit 1
fi
...
printf 'Inspecting ChatGPT from %s %s (Git %s)...\n' \
    "$package_name" "$package_version" "${built_commit:-$checkout_commit}"
...
printf '\nFrameless patch installed for ChatGPT (%s %s).\n' "$package_name" "$package_version"
printf 'After the next Git rebuild/install, fully quit ChatGPT and run this same script again.\n'
